    ...
    def generate_target_materials(self, cut_kernel = None, radius=None, radius_list = None,
        ...
        if radius == 'nocut':
            ...
            if radius == None:
                ...
                if detect:
                    ...
                    detection_dict = {}
                    detection_dict['segm_deblend'] = segm_deblend
                    detection_dict['tbl'] = tbl

                    with open('%s/detection_dict.pkl' %detection_path, 'wb') as handle:
                        pkl.dump(detection_dict, handle, protocol=pkl.HIGHEST_PROTOCOL)

                if not detect:
                    with open('%s/detection_dict.pkl' %detection_path, 'rb') as handle:
                        detection_dict = pkl.load(handle)

                    segm_deblend = detection_dict['segm_deblend']
                    tbl = detection_dict['tbl']
        ...
        if skip == False:
            ...
            if detect:
                ...
                detection_dict = {}
                detection_dict['apertures'] = apertures
                detection_dict['segm_deblend'] = segm_deblend
                detection_dict['mask_apertures'] = mask_apertures
                detection_dict['tbl'] = tbl
            ...
            if not detect:
                ...
                segm_deblend = detection_dict['segm_deblend']
                mask_apertures = detection_dict['mask_apertures']
                tbl = detection_dict['tbl']
                ...
                if nearyby_obj_filter:
                    ...
                    for i in range(len(PSF_cutouts)):
                        ...
                        if detect:
                            ...
                            detection_dict = {}
                            detection_dict['_detect'] = _detect
                        ...
                        if not detect:
                            ...
                            _detect = detection_dict['_detect']
